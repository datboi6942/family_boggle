...
class Challenge:
    ...
    def get_progress_ratio(self, found_words: list[str], score: int) -> float:
        ...
        return (
            min(self.check_progress(found_words, score) / self.target, 1.0)
            if self.target > 0
            else 0.0
        )
...
class PalindromeChallenge(Challenge):
    ...
    def check_progress(self, found_words: list[str], score: int) -> int:
        return sum(
            1 for w in found_words if w.upper() == w.upper()[::-1] and len(w) >= 3
        )
...
class VowelHeavyChallenge(Challenge):
    ...
    def check_progress(self, found_words: list[str], score: int) -> int:
        ...
        return sum(
            ...
            if sum(1 for c in w.upper() if c in vowels) >= self.min_vowels
        )
...
class RareLetterChallenge(Challenge):
    ...
    def check_progress(self, found_words: list[str], score: int) -> int:
        return sum(
            1 for w in found_words if any(c in self.rare_letters for c in w.upper())
        )
...
ALL_CHALLENGES: list[Challenge] = [
    ...
    WordCountChallenge(
        "words_12", "Word Seeker", "Find 12 words", 12, "words", "easy", 20
    ),
    WordLengthChallenge(
        "long_5_3", "Extended Words", "Find 3 words with 5+ letters", 3, 5, "easy", 25
    ),
    ScoreChallenge("score_75", "Point Pursuer", "Score 75 points", 75, "easy", 20),
    WordEndsWithChallenge(
        "ends_s_5", "Plural Hunter", "Find 5 words ending in S", 5, "S", "easy", 25
    ),
    DoubleLetterChallenge(
        "double_3", "Double Trouble", "Find 3 words with double letters", 3, "easy", 30
    ),
    WordEndsWithChallenge(
        "ends_ed_3", "Past Tense", "Find 3 words ending in ED", 3, "ED", "easy", 25
    ),
    # ==================== MEDIUM (35-55 points) ====================
    # Require solid gameplay and strategy
    WordCountChallenge(
        "words_18", "Vocabulary Vault", "Find 18 words", 18, "words", "medium", 40
    ),
    WordLengthChallenge(
        "long_6_3", "Six Letter Pro", "Find 3 words with 6+ letters", 3, 6, "medium", 45
    ),
    ScoreChallenge("score_125", "Point Prodigy", "Score 125 points", 125, "medium", 40),
    WordEndsWithChallenge(
        "ends_ing_3", "ING Master", "Find 3 words ending in ING", 3, "ING", "medium", 50
    ),
    VowelHeavyChallenge(
        "vowels_4", "Vowel Hunter", "Find 4 words with 3+ vowels", 4, 3, "medium", 45
    ),
    DoubleLetterChallenge(
        "double_5", "Twin Terms", "Find 5 words with double letters", 5, "medium", 50
    ),
    WordEndsWithChallenge(
        "ends_er_4", "ER Expert", "Find 4 words ending in ER", 4, "ER", "medium", 40
    ),
    WordLengthChallenge(
        "long_7_1", "Lucky Seven", "Find 1 word with 7+ letters", 1, 7, "medium", 35
    ),
    # ==================== HARD (60-80 points) ====================
    # Challenging - require excellent vocabulary and board scanning
    WordCountChallenge(
        "words_25", "Word Wizard", "Find 25 words", 25, "words", "hard", 70
    ),
    WordLengthChallenge(
        "long_6_5", "Lengthy Legend", "Find 5 words with 6+ letters", 5, 6, "hard", 75
    ),
    WordLengthChallenge(
        "long_7_2", "Marathon Words", "Find 2 words with 7+ letters", 2, 7, "hard", 70
    ),
    ScoreChallenge(
        "score_175", "Point Powerhouse", "Score 175 points", 175, "hard", 65
    ),
    WordEndsWithChallenge(
        "ends_ly_2", "LY Master", "Find 2 words ending in LY", 2, "LY", "hard", 70
    ),
    WordEndsWithChallenge(
        "ends_tion_1",
        "TION Station",
        "Find 1 word ending in TION",
        1,
        "TION",
        "hard",
        75,
    ...
    VowelHeavyChallenge(
        "vowels_4_4", "Super Vowel", "Find 4 words with 4+ vowels", 4, 4, "hard", 80
    ),
    ConsonantHeavyChallenge(
        "consonants_2",
        "Consonant Crusher",
        "Find 2 words with 4+ consonants in a row",
        2,
        "hard",
        75,
    ),
    RareLetterChallenge(
        "rare_1", "Rare Find", "Find 1 word with Q, X, Z, or J", 1, "QXZJ", "hard", 65
    ),
    PalindromeChallenge(
        "palindrome_1", "Mirror Mirror", "Find 1 palindrome word", 1, "hard", 80
    ),
    DoubleLetterChallenge(
        "double_7", "Double Vision", "Find 7 words with double letters", 7, "hard", 70
    ),
    # ==================== VERY HARD (90-100 points) ====================
    # Elite challenges - require exceptional performance
    WordCountChallenge(
        "words_35", "Lexicon Lord", "Find 35 words", 35, "words", "very_hard", 95
    ),
    WordLengthChallenge(
        "long_7_4",
        "Seven Samurai",
        "Find 4 words with 7+ letters",
        4,
        7,
        "very_hard",
        100,
    ...
    WordLengthChallenge(
        "long_8_1", "Eight Wonder", "Find 1 word with 8+ letters", 1, 8, "very_hard", 90
    ),
    ScoreChallenge(
        "score_250", "Quarter King", "Score 250 points", 250, "very_hard", 95
    ),
    WordStartsWithChallenge(
        "starts_q_1", "Q-Quest", "Find 1 word starting with Q", 1, "Q", "very_hard", 95
    ),
    WordStartsWithChallenge(
        "starts_x_1",
        "X-Factor",
        "Find 1 word starting with X",
        1,
        "X",
        "very_hard",
        100,
    ...
    WordStartsWithChallenge(
        "starts_z_1", "Z-Zone", "Find 1 word starting with Z", 1, "Z", "very_hard", 95
    ),
    VowelHeavyChallenge(
        "vowels_5_4",
        "Quad Vowel Master",
        "Find 5 words with 4+ vowels",
        5,
        4,
        "very_hard",
        100,
    ...
    ConsonantHeavyChallenge(
        "consonants_3",
        "Consonant King",
        "Find 3 words with 4+ consonants in a row",
        3,
        "very_hard",
        95,
    ),
    ...
    PalindromeChallenge(
        "palindrome_2", "Mirror Master", "Find 2 palindrome words", 2, "very_hard", 100
    ),
    WordEndsWithChallenge(
        "ends_ing_5",
        "ING King",
        "Find 5 words ending in ING",
        5,
        "ING",
        "very_hard",
        90,
...
class PlayerChallengeProgress:
    ...
    def get_best_challenge(self, found_words: list[str], score: int) -> dict | None:
...
class ChallengeManager:
    ...
    def get_player_progress(
        self, lobby_id: str, found_words: list[str], score: int
    ) -> list[dict]:
